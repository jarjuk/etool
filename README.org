...
To achive isolation width src_bash[:exports results]{echo -n
${ISOLATION_WIDTH}mm }, when carving ~3~ grooves using milling bit
with cutting width src_bash[:exports results]{echo -n
${MILL_DIAMETERS}mm } requires


#+name: 3-grooves
#+BEGIN_SRC python :eval no-export :results output :noweb no :session *Python* :exports results
  print( f"{overLap(i=ISOLATION_WIDTH, cut=MILL_DIAMETERS, n=3):.1f}%")
#+END_SRC
