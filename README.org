#+PROPERTY: header-args+ :var  ETOOL_DOCKER_TAG="7"
...
#+begin_example
         ...
         o cleanup               : remove files from /etool -data directories
                                   /etool/01-gerber, /etool/01-image, /etool/02-ngc, /etool/02-silk
...
2) Gerber to gcode and image conversion
...
2.2) Output produced with ~gerber~ -command in ~marcus2002/etool~:
     gcode files in *02-ngc* subdirectory.

2.3) Output produced with ~gerber~ -command in ~marcus2002/etool~: png
...
3.2) Output produced with ~image~ -command in ~marcus2002/etool~: gcode
